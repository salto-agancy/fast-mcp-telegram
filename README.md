...
| :closed_lock_with_key: **[Multi-User Authentication](docs/Installation.md#remote-setup-http-auth)** | Production-ready Bearer token auth with session isolation and LRU cache management                             |
| :label: **[Multi-Account Tool Prefix](docs/Installation.md#multi-account-mcp-tool-prefix)**          | Optional `PREFIX_MCP_TOOLS_WITH_USERNAME` — prefixes tool names per session (username or user id) for multi-account agents on one server |
