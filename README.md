...
| :shield: **[Session ACL](docs/Installation.md#session-acl-http-auth)** | Opt-in per-token limits on `http-auth` (`ACL_ENABLED`) — allowed chats, `read_only`, `blocked_peers`; see [SECURITY.md](SECURITY.md#opt-in-session-acl-http-auth) |
