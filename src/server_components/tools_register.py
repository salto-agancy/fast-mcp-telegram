...
_DESC_SEND_RICH_MESSAGE = _tool_description(
    ...
    "layout that should render natively in Telegram. Sent by the Salto bot, so "
    "chat_id must be a chat shared with that bot (e.g. the bot DM 774060215), "
            ...
            try:
                try:
                    ...
                    raise RuntimeError(
                        ...
                        f"{_budget:.0f}s. Do not blind-retry or wait: вызови скилл "
                        f"fix-telegram-mcp (симптом TOOL_TIMEOUT)."
                    ) from None
                except Exception:
