### 2026-05-19
- **0.18.1 / FastMCP 3.3 slim**: Production dependency `fastmcp-slim[server]>=3.3` replaces `fastmcp` metapackage (3.2.4). Lock upgraded with `uv lock --upgrade`. 482 tests pass. GitHub release `0.18.1`; GHCR `:main` image size to be compared post-deploy vs pre-0.18.1 baseline (~39.3 MiB compressed).
